...
e.values.forEach(function(e,i){var o=Math.floor(i/Y)+2
...
return t[0]=e[0]/w-W[0]/w,t[1]=e[1]/w-A-W[1]/w,t}function i(){E.forEach(function(e,t){var n
n=e.x1-e.sprite.position.x,Math.abs(n)>.1&&(e.sprite.position.x+=.1*n),n=e.y1-e.sprite.position.y,Math.abs(n)>.1&&(e.sprite.position.y+=.1*n),n=e.alpha-e.sprite.alpha,Math.abs(n)>.01&&(e.sprite.alpha+=.2*n),e.sprite.visible=e.sprite.alpha>.1,e.sprite2&&(n=e.alpha2-e.sprite2.alpha,Math.abs(n)>.01&&(e.sprite2.alpha+=.2*n),e.sprite2.visible=e.sprite2.alpha>.1)})}function o(e){requestAnimationFrame(o),y(),i(),k.render(z),M.update()}function r(e,t){xe.zoomingToImage=!0,G.center(null),x(e),d3.select(".tagcloud").classed("hide",!0)
...
if(!ge){E.forEach(function(t,n){var a=t.sprite.position,i=a.x/L+W[0]/e,o=a.y/L+W[1]/e,r=5
i>-r&&j/e+r>i&&A/e+r>o+A&&o>-1*A-r?t.visible=!0:t.visible=!1})
var t=E.filter(function(e){return e.visible})
t.length<50?E.forEach(function(e){e.visible&&e.loaded&&e.active?e.alpha2=1:e.visible&&!e.loaded&&e.active?N.push(e):e.alpha2=0}):E.forEach(function(e){e.alpha2=0})}}function x(e){if(e.loaded)return void(e.alpha2=1)
...
i.anchor.x=.5,i.anchor.y=.5,i.position.x=e.x*D+J/2,i.position.y=e.y*D+J/2,i._data=e,de.addChild(i),e.sprite2=i,e.alpha2=e.highlight}),e.loaded=!0,t.src="data:image/jpg;base64,"+imagesMap2.get(e.id).image}}function y(){if(!ke&&!ge&&N.length){var e=N.pop()
...
e.resize=function(){j=window.innerWidth-S.left-S.right,A=window.innerHeight<B?B:window.innerHeight,q=window.innerWidth,k.resize(j+S.left+S.right,A),e.makeScales(),e.flip()},e.makeScales=function(){F.rangeBands([S.left,j+S.left],.2),K=F.rangeBand(),U=F.rangeBand()/3,X=K/Y/2,L=V/(F.rangeBand()/Y),D=J/(F.rangeBand()/Y),Z=Q/(F.rangeBand()/Y),ce.scale.x=1/L,ce.scale.y=1/L,ce.y=A,de.scale.x=1/D,de.scale.y=1/D,de.y=A,ue.scale.x=1/Z,ue.scale.y=1/Z,ue.y=A,H.forEach(function(e){e.x=F(e.key)})},e.init=function(n,a){E=n,b=d3.select(".page").append("div").classed("viz",!0),PIXI.SCALE_MODES.DEFAULT=PIXI.SCALE_MODES.NEAREST
var i={transparent:!1,resolution:1,antialiasing:!1}
k=new PIXI.WebGLRenderer(j+S.left+S.right,A,i),k.backgroundColor=1842719,window.renderer=k
var l=d3.select(b.node().appendChild(k.view))
M=new Stats,document.body.appendChild(M.domElement),z=new PIXI.Container,se=new PIXI.Container,ce=new PIXI.Container,de=new PIXI.Container,ue=new PIXI.Container,z.addChild(se),se.addChild(ce),se.addChild(de),se.addChild(ue),E.forEach(function(e,t){var n=PIXI.Texture.fromImage("data:image/jpg;base64,"+imagesMap.get(e.id).image),a=new PIXI.Sprite(n)
...
var he=null,ge=!1,ve=117,me=2,xe={lastZoomed:0,zoomingToImage:!1},ye=d3.scale.linear().domain([1,9]).range([9,20]).clamp(!0),we=(d3.scale.linear().domain([2,8,20]).range([9,14,19]).clamp(!0),d3.scale.threshold().domain([3,10,20]).range(["none","small","middle","large"])),Ie=!1,be=[0,0],Ee=0,ke=!1
return e.highlight=function(){E.forEach(function(e,t){e.alpha=e.highlight?1:.2})},e.resetZoom=function(){var e=1400
ne=d3.extent(E,function(e){return e.y})
var t=-ne[1]-te
ae=-30>t&&t>-40,C.call(G.translate(W).event).transition().duration(e).call(G.scale(1).translate([0,t]).event)},e.flip=function(){e.split(),e.resetZoom()},e.split=function(){var e=E.filter(function(e){return e.active})
n(e,!1)
